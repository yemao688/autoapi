...
function createDraft(view: service.ToolProviderView | null, key: DraftKey, isNew = false): ProviderDraft {
  ...
  return {
    ...
    vendor: kind === 'autoapi' && !['openai-responses', 'openai-compatible', 'anthropic', 'google-gemini'].includes(savedVendor) ? 'openai-compatible' : savedVendor,
          ...
          <div class="opencode-workbench-body">
            ...
            <main class="opencode-editor">
              <template v-if="section === 'provider' && selectedDraft">
                ...
                <template v-else><div class="field"><label class="field-label">{{ t('toolAccess.presets.interfaceFormat') }}</label><select v-model="selectedDraft.vendor" class="select" :disabled="selectedIsPendingDelete"><option value="openai-responses">{{ t('toolAccess.vendors.openai-responses') }}</option><option value="openai-compatible">{{ t('toolAccess.vendors.openai-compatible') }}</option><option value="anthropic">{{ t('toolAccess.vendors.anthropic') }}</option><option value="google-gemini">{{ t('toolAccess.vendors.google-gemini') }}</option></select><div class="field-help">{{ t('toolAccess.preset.vendorHelp.' + (selectedDraft.vendor || 'openai-compatible')) }}</div></div><div class="field"><label class="field-label">{{ t('toolAccess.preset.apiKeySelector') }}</label><select v-model="selectedDraft.apiKeyID" class="select" :disabled="supportingLoading || selectedIsPendingDelete"><option value="" disabled>{{ t('toolAccess.preset.apiKeyPlaceholder') }}</option><option v-for="key in apiKeys" :key="key.id" :value="key.id">{{ key.name }}</option></select><div class="field-help">{{ supportingLoading ? t('toolAccess.preset.loadingSupporting') : t('toolAccess.preset.apiKeyHelp') }}</div></div></template>
