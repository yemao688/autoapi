...
func AttributeAttempt(key model.TargetMetricKey, outcome model.AttemptOutcome, status int, committed bool, firstByteMs, ttftMs int64) model.TargetMetricEvent {
	e := model.TargetMetricEvent{Key: key, Kind: model.MetricEventAttempt, AttemptOutcome: outcome, StatusCode: status, StreamCommitted: committed, FirstByteMs: firstByteMs, TTFTMs: ttftMs}
	switch {
